...
国际自动来源现包括携程、同程、去哪儿、飞猪、Google Flights、Kiwi.com 和瑞安航空，全部使用匿名公开网页或网页服务，无需用户申请密钥、安装浏览器或填写 token。同程、飞猪、Google 及已标明含税的去哪儿日历可参与总价提醒；Kiwi 与瑞安保留为参考展示，不将口径不确定的低价用于阈值。同程和飞猪的字段与请求验证分别见 [TONGCHENG_SOURCE.md](TONGCHENG_SOURCE.md) 和 [SOURCES_FLIGGY.md](SOURCES_FLIGGY.md)。
...
2026-09-14 更新：结果页已为 Trip.com、Skyscanner、KAYAK、momondo、春秋航空和 AirAsia 生成带城市代码及最早出发日期的官方核价链接；同程、飞猪国际报价也提供精确入口。额外入口默认整体收起。由于下面列出的验证或动态加载限制，额外入口本身不计入自动返回报价的平台数，也不会影响最低价或提醒。
